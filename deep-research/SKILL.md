...
## Codex-Native Multi-Agent Contract

- This skill targets Codex native multi-agent tools, not abstract `Task(...)` APIs.
- When the user explicitly asks for deep research, comprehensive analysis, comparisons, trend analysis, sub-agents, delegation, or parallel work, you may use `spawn_agent`.
- Prefer `spawn_agent` for bounded sidecar work, `assign_task` for redirected follow-up work, `send_message` for queued clarifications, `wait_agent` for blocking joins, `list_agents` for visibility, and `close_agent` when the subtree is no longer needed.
- Prefer 2-4 subagents with disjoint scopes. Avoid recursive fan-out unless the active session has depth headroom and the additional split is clearly worthwhile.
- Require every spawned agent to return structured evidence objects or a tightly scoped synthesis, not open-ended prose dumps.
