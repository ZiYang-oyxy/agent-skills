...
Enterprise-grade research engine for Codex. Produces citation-backed reports with source credibility scoring, multi-provider search, native multi-agent coordination, and automated validation.

## Installation

```bash
# Clone into a local skills directory
...
- **Parallel retrieval**: 5-10 concurrent searches + 2-4 focused sub-agents returning structured evidence objects
...
- **Codex-native orchestration**: `spawn_agent`, `assign_task`, `wait_agent`, `list_agents`, and `close_agent`
...
Reports >18K words auto-continue via native agent spawning with context preservation.
