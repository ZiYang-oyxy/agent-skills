...
**Codex-native rule:** Use built-in parallel tool calls for independent retrieval. For multi-agent work, use `spawn_agent` to create 2-4 focused research workers, `list_agents` to inspect live state when needed, `assign_task` for redirected follow-ups, `wait_agent` only when blocked on results, and `close_agent` once an agent is no longer needed.
...
Use `spawn_agent` with 2-4 focused agents for:
...
- One clear scope only
- Explicit source and time bounds when relevant
- A structured output contract
- A reminder not to duplicate sibling agents' scopes
...
**Example Codex-native fan-out pattern:**
```
1. Launch independent search queries in one parallel tool round.
2. Spawn bounded agents:
   - `spawn_agent(task_name="academic_analysis", agent_type="default", message="Analyze academic papers from [CURRENT_YEAR] for key findings, methods, and limitations. Return 5-10 JSON evidence objects only.")`
   - `spawn_agent(task_name="industry_analysis", agent_type="default", message="Analyze industry reports and market data for commercial applications, adoption blockers, and timelines. Return JSON evidence objects only.")`
   - `spawn_agent(task_name="technical_challenges", agent_type="default", message="Extract technical limitations, failure modes, and open problems. Return JSON evidence objects only.")`
3. Continue local synthesis while those agents run.
4. Call `wait_agent(targets=[...])` only when you need the next completed result to move forward.
5. If one agent needs a narrower follow-up, use `assign_task(target="technical_challenges", items=[...])` instead of spawning an overlapping sibling.
```
...
- `spawn_agent(task_name="academic_analysis", message="Analyze the academic corpus surfaced in this retrieval round. Return structured evidence objects only.")`
```
...
- Use `spawn_agent` for parallel retrieval or verification agents (2-4 agents)
...
Use `spawn_agent` to create sub-agents for:
