...
3. Spawn continuation agent via `spawn_agent`
4. Continuation agent: Reads state -> Generates next batch -> Reuses `assign_task` or spawns next if needed
...
**Location:** `~/.codex/research_output/continuation_state_[report_id].json`
...
```
spawn_agent(
  task_name="report_continuation_01",
  agent_type="default",
  message="""
CONTINUATION TASK: Continue existing deep-research report.

CRITICAL INSTRUCTIONS:
1. Read continuation state: ~/.codex/research_output/continuation_state_[report_id].json
...
Use native file editing tools to append to: [file_path]
...
After generating:
- If more sections remain: Update state, then either `assign_task` to the current continuation agent if it should keep ownership, or spawn the next continuation agent with a non-overlapping handoff
